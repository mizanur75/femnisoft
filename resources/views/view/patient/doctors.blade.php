				...
				<table id="tableId" class="table table-sm table-bordered table-striped">
					...
					@foreach($doctors as $doctor)
						<tr>
							...
							<form action="{{ route('agent.sendrequest',['doctor_id'=>$doctor->id, 'patient_id'=>$patient->id]) }}" class="sendrequest" method="GET" id="selectdate{{$doctor->id}}">
								@csrf
								...
								<td class="text-center">
								<button type="button" onclick="selectdate('{{$doctor->id}}')" id="{{$doctor->id}}" class="btn btn-danger btn-sm d-none"><i class="fa fa-paper-plane" value="2"></i> Send Request</button>
								</td>
							</form>
						</tr>
					@endforeach
